...
===
with tbl_cached as (select UDT1.ntid,Course1.courseId,CMR1.ps
...
Course1.deleted=0 and UMR1.idFunction='F7' And UDT1.SFunction='GWO(wells)'
group by UDT1.ntid,Course1.courseId,CMR1.ps
)
select UD.ntid, (UD.fname+','+UD.lname) As Fullname ,UD.fname,UD.lname,UD.omsSubfunction,
	Cr.courseArena,rpc.courseId,
Cr.courseTitle,Cr.courseType,
Cr.courseDuration,rpc.ps,Cr.courseDelivery,rpc.count_conflict	
	from (((user_data as UD 
		inner join 
	(select UDT.ntid, Course.courseId,CMR.ps,
(select count (*) from tbl_cached where tbl_cached.ntid = UDT.ntid and tbl_cached.courseId = Course.courseId)
 as count_conflict
...
Course.deleted=0 and UMR.idFunction='F7' And UDT.SFunction='GWO(wells)'
